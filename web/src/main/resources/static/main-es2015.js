...
let FiltersComponent = class FiltersComponent {
    ...
    addNewFilter(title) {
        if (title) {
            this.filters.push({ serialNumber: this.filters.length, title });
            this.filterToSend = [];
            for (let i = 0; i < this.filters.length; i++) {
                this.filterToSend[i] = this.filters[i].title;
            }
            ...
            if (event instanceof _angular_router__WEBPACK_IMPORTED_MODULE_3__["NavigationStart"]) {
                ...
                if (url.includes('companies')) {
                    ...
                    if (url.includes('products')) {
                        ...
                        this.urlForProduct = url.substring(0, url.indexOf('products') + 8);
                    }
                    else {
                        this.activeProduct = false;
                    }
                    if (url.includes('programs')) {
                        ...
                        this.urlForProgram = url.substring(0, url.indexOf('programs') + 8);
                    }
                    else {
                        this.activeProgram = false;
                    }
                    if (url.includes('coverages')) {
                        ...
                        this.urlForCoverage = url.substring(0, url.indexOf('coverages') + 9);
                    }
                    else {
                        this.activeCoverage = false;
                    }
...
let CompanyIdComponent = class CompanyIdComponent {
    constructor(_settingsService, router) {
    ...
    updateCurrentCompany(company) {
        ...
        if (this.currentCompany.contactPersons.length === 1 && this.currentCompany.mainContactPersonId === undefined) {
            ...
            for (const contact of this.currentCompany.contactPersons) {
            ...
            for (const payment of this.currentCompany.paymentDetails) {
    ...
    updateCompany() {
        if (this.idOfCompany[0] === 'n') {
            ...
            this._settingsService.getCompanyById(this.idOfCompany).subscribe(data => {
    ...
    changeCompany() {
        ...
        this._settingsService.postCompanyById(companyToSend).subscribe(data => {
            if (this.newCompany === true) {
                this.router.navigate(['../companies/', data], { replaceUrl: true });
                this.setShowButtons(true);
                this.currentCompany.id = data;
            }
        }, error => {
            console.log(error);
        });
    ...
    // contact
    sortContacts() {
        this.currentCompany.contactPersons.sort((a, b) => (a.main < b.main) ? 1 : ((b.main < a.main) ? -1 : 0));
    }
    changeMainContact(choosenContact) {
    ...
    sendContact(contact) {
        ...
        this._settingsService.postContact(contactToSend).subscribe(data => {
            this.currentCompany.contactPersons[0].id = data;
            if (this.currentCompany.contactPersons.length === 1) {
                this.currentCompany.mainContactPersonId = data;
                this.changeCompany();
            }
            this.setShowButtons(true);
        }, error => {
    ...
    deleteContact(choosenContact) {
        const idTodelete = choosenContact.id;
        if (idTodelete === 0) {
            this.updateCompany();
            this.setShowButtons(true);
        }
        this._settingsService.deleteContact(idTodelete).subscribe(data => {
            this.updateCompany();
            this.setShowButtons(true);
        }, error => {
            console.log(error);
    ...
    sendPayment(payment) {
        ...
        this._settingsService.postPayment(paymentToSend).subscribe(data => {
            this.currentCompany.paymentDetails[0].id = data;
            this.setShowButtons(true);
        }, error => {
            console.log(error);
        });
    }
    deletePayment(choosenPayment) {
        const idTodelete = choosenPayment.id;
        if (idTodelete === 0) {
            for (const payment of this.currentCompany.paymentDetails) {
                if (payment.id === choosenPayment.id) {
                    this.currentCompany.paymentDetails = this.currentCompany.paymentDetails.filter((obj) => obj !== payment);
                }
            }
            this.setShowButtons(true);
        }
        else {
            this._settingsService.deletePayment(idTodelete).subscribe(data => {
                for (const payment of this.currentCompany.paymentDetails) {
                    if (payment.id === choosenPayment.id) {
                        this.currentCompany.paymentDetails = this.currentCompany.paymentDetails.filter((obj) => obj !== payment);
                    }
...
let CompanyListComponent = class CompanyListComponent {
    constructor(_settingsService, router, _dateToSendService) {
        this._settingsService = _settingsService;
        this.router = router;
    ...
    deleteCompany() {
        for (const company of this.companiesList) {
            if (company.selected === true) {
                this._settingsService.deleteCompanyById(company.id).subscribe(data => {
                    this.updateList();
                }, error => {
                    console.log(error);
                });
    ...
    updateList() {
        this._settingsService.getCompanyList(this.filtersToSend).subscribe(data => {
            this.companiesList = data.content;
            this.amountOfCompanies = data.content.length;
            this.typeOfLength = this._dateToSendService.checkAmountToShow(this.amountOfCompanies);
            for (const company of this.companiesList) {
...
let CoverageListMedAddComponent = class CoverageListMedAddComponent {
    constructor(_mkb10Service, _settingsservice) {
        this._mkb10Service = _mkb10Service;
        this._settingsservice = _settingsservice;
    ...
    addNewMed() {
        ...
        this._settingsservice.postMedicamentForCoverageById(arrayToSend, this.programId).subscribe(data => {
            this.med = {
                programCoverId: this.coverageId,
                medicamentGroupId: '',
                onceFranchise: {
    ...
    ngOnInit() {
        ...
        this._mkb10Service.getListOfMedsGroup().subscribe(data => {
            this.medicamentGroups = data.content;
        }, error => {
            console.log(error);
        });
    }
...
let CoverageListMedComponent = class CoverageListMedComponent {
    constructor(_settingsservice, _mkb10Service) {
        this._settingsservice = _settingsservice;
        this._mkb10Service = _mkb10Service;
        this.onChanged = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
        this.med = {
    ...
    deleteMed() {
        this._settingsservice.deleteMedicamentForCoverageById(this.coverageId, this.med.id).subscribe(data => {
            this.changeCoverage(data);
        }, error => {
            console.log(error);
        });
    }
    ...
    ngOnInit() {
        ...
        for (const field in this.medicament) {
...
let CoverageListProcedureAddComponent = class CoverageListProcedureAddComponent {
    constructor(_mkb10Service, _settingsservice) {
        this._mkb10Service = _mkb10Service;
        this._settingsservice = _settingsservice;
        this.onCancel = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
    ...
    addNewService() {
        ...
        this._settingsservice.postProcedureForCoverageById(arrayToSend, this.programId).subscribe(data => {
            this.service = {
                programCoverId: this.coverageId,
                procedureGroupId: '',
                onceFranchise: {
    ...
    ngOnInit() {
        ...
        this._mkb10Service.getListOfServiceGroup().subscribe(data => {
            this.procedureGroups = data.content;
        }, error => {
            console.log(error);
        });
    }
...
let CoverageListProcedureComponent = class CoverageListProcedureComponent {
    constructor(_settingsservice, _mkb10Service) {
        this._settingsservice = _settingsservice;
        this._mkb10Service = _mkb10Service;
        this.onChanged = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
    ...
    deleteService() {
        this._settingsservice.deleteProcedureForCoverageById(this.coverageId, this.procedure.id).subscribe(data => {
            this.changeCoverage(data);
        }, error => {
            console.log(error);
        });
    ...
    ngOnInit() {
        ...
        for (const field in this.service) {
...
let CoverageListComponent = class CoverageListComponent {
    constructor(router, _mkb10Service, _settingsservice) {
        this.router = router;
        this._mkb10Service = _mkb10Service;
        this._settingsservice = _settingsservice;
    ...
    changeCoverage(coverage) {
        ...
        this._settingsservice.postCoverageById(coverage).subscribe(data => {
            for (let cover of this.coverageList) {
    ...
    changeMedicament(idOfCoverage, medicamentGroup) {
        for (const coverage of this.coverageList) {
            if (idOfCoverage === coverage.id) {
                for (let medicament of coverage.medicamentGroups) {
    ...
    addNewMedicament(coverage) {
        coverage.isAddingMed = true;
        this.showAddBtn = false;
    }
    cancelAddingMed(coverage) {
        coverage.isAddingMed = false;
        this.showAddBtn = true;
    }
    changeCoverageLocal(idOfCoverage, data) {
    ...
    addNewProcedure(coverage) {
        coverage.isAddingService = true;
        this.showAddBtn = false;
    }
    cancelAddingProcedure(coverage) {
        coverage.isAddingService = false;
        this.showAddBtn = true;
    }
    updateList() {
        this._settingsservice.getCoveragesListByProgram(this.idOfProgram).subscribe(data => {
    ...
    ngOnInit() {
        ...
        this.updateList();
        this._settingsservice.getProgramById(this.idOfProgram).subscribe(data => {
            this.nameOfProgram = data.name;
        }, error => {
            console.log(error);
        });
    }
...
let JavaHackStartPageComponent = class JavaHackStartPageComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let LossIdMedAddComponent = class LossIdMedAddComponent {
    constructor(_mkb10Service, _dateToSend, _losesservice, router) {
        this._mkb10Service = _mkb10Service;
        ...
        this._mkb10Service.getListOfMeds().subscribe(data => {
            for (const value of data.content) {
                this.optionsForMeds.push(value);
                this.optionsForMedsCode.push(value);
            }
            this.filteredOptionsForMeds = this._dateToSend.makeListForFilters(this.controlForMeds, 'name', this.optionsForMeds, 3);
            this.filteredOptionsForMedsCode = this._dateToSend.makeListForFilters(this.controlForMedsCode, 'code', this.optionsForMedsCode, 3);
            this.showSpinerMed = false;
        }, error => { console.log(error); });
        this._mkb10Service.getListOfMedsGroup().subscribe(data => {
            for (const value of data.content) {
                this.optionsForMedGroup.push(value);
            }
            this.filteredOptionsForMedGroup = this._dateToSend.makeListForFilters(this.controlForMedGroup, 'name', this.optionsForMedGroup, 0);
            this.showSpinerMedGroup = false;
        }, error => { console.log(error); });
    }
    checkModalMedSave() {
    ...
    addNewMed() {
        ...
        this._losesservice.addNewMed(this.idOfLoss, arrayToSend).subscribe(data => {
            this.onChanged.emit(data);
            this.modalMed = new _interface_and_class_forloss_class__WEBPACK_IMPORTED_MODULE_3__["ModalMed"]();
            this.clearModal();
        }, error => { console.log(error); });
...
let LossIdMedComponent = class LossIdMedComponent {
    constructor(_losesservice, _dateToSend, router) {
        this._losesservice = _losesservice;
        this._dateToSend = _dateToSend;
        this.router = router;
        this.onChanged = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
    ...
    saveToModalTempMed(medOrService) {
    ...
    changeStatusMed(id, medId, status) {
        const statusToSend = {
            stateId: status
        };
        this._losesservice.changeStatusMed(id, medId, statusToSend).subscribe(data => {
            this.onChanged.emit(data);
        }, error => { console.log(error); });
    }
    changeMed(med) {
    ...
    deleteMed(idOfMed) {
        this._losesservice.deleteMed(this.idOfLoss, idOfMed).subscribe(data => {
            this.onChanged.emit(data);
        }, error => { console.log(error); });
    }
    ngOnInit() {
...
let LossIdMkbModalComponent = class LossIdMkbModalComponent {
    constructor(_mkb10Service, router) {
        this._mkb10Service = _mkb10Service;
        this.router = router;
        this.onChanged = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
        ...
        this.treeFlattener = new _angular_material_tree__WEBPACK_IMPORTED_MODULE_3__["MatTreeFlattener"](this.transformer, node => node.level, node => node.expandable, node => node.nodes);
        ...
        this._mkb10Service.getMkb10().subscribe(data => {
...
let LossIdProcedureAddComponent = class LossIdProcedureAddComponent {
    constructor(_mkb10Service, _dateToSend, _losesservice, router) {
        this._mkb10Service = _mkb10Service;
        this._dateToSend = _dateToSend;
        this._losesservice = _losesservice;
        ...
        this._mkb10Service.getListOfService().subscribe(data => {
            for (const value of data.content) {
                this.optionsForService.push(value);
                this.optionsForServiceCode.push(value);
            }
            this.filteredOptionsForService = this._dateToSend.makeListForFilters(this.controlForService, 'name', this.optionsForService, 3);
            this.filteredOptionsForServiceCode = this._dateToSend.makeListForFilters(this.controlForServiceCode, 'code', this.optionsForServiceCode, 3);
            this.showSpinerService = false;
        }, error => { console.log(error); });
        this._mkb10Service.getListOfServiceGroup().subscribe(data => {
            for (const value of data.content) {
                this.optionsForServiceGroup.push(value);
            }
            this.filteredOptionsForServiceGroup = this._dateToSend.makeListForFilters(this.controlForServiceGroup, 'name', this.optionsForServiceGroup, 0);
            this.showSpinerServiceGroup = false;
        }, error => { console.log(error); });
    }
    checkModalServiceSave() {
    ...
    chooseServiceFromListCode(option) {
    ...
    addNewService() {
        ...
        this._losesservice.addNewService(this.idOfLoss, arrayToSend).subscribe(data => {
            this.onChanged.emit(data);
            this.modalService = new _interface_and_class_forloss_class__WEBPACK_IMPORTED_MODULE_3__["ModalService"]();
            this.clearModal();
        }, error => { console.log(error); });
...
let LossIdProcedureComponent = class LossIdProcedureComponent {
    constructor(_losesservice, _dateToSend, router) {
        this._losesservice = _losesservice;
        this._dateToSend = _dateToSend;
        this.router = router;
    ...
    cancelModalService(medOrService) {
    ...
    saveToModalTempService(medOrService) {
    ...
    changeStatusService(id, medId, status) {
        const statusToSend = {
            stateId: status
        };
        this._losesservice.changeStatusService(id, medId, statusToSend).subscribe(data => {
            this.onChanged.emit(data);
        }, error => { console.log(error); });
    }
    ...
    deleteService(idOfService) {
        this._losesservice.deleteService(this.idOfLoss, idOfService).subscribe(data => {
            this.onChanged.emit(data);
        }, error => { console.log(error); });
    }
    ngOnInit() {
...
let LossIdComponent = class LossIdComponent {
    constructor(_losesservice, _mkb10Service, _usersService, _router, _dateToSend) {
        this._losesservice = _losesservice;
        this._mkb10Service = _mkb10Service;
        ...
        if (this.idOfLoss[0] === 'n') {
            ...
            this._usersService.getUserById(this.idOfLoss.slice(4)).subscribe(data => {
                const today = new Date();
                this.loss.date = this._dateToSend.makeDateToSend(today);
                this.loss.id = data.id;
                this.loss.assured = data;
                this.loss.stateId = 1;
            }, error => { console.log(error); });
            this._losesservice.getAllLossesForUser(this.idOfLoss.slice(4)).subscribe(data => {
                this.idOfLoss = null;
                this.lossListForUser = data;
            }, error => { console.log(error); });
        }
        else {
            this._losesservice.getLossesById(this.idOfLoss).subscribe(data => {
                this.loss = data;
                this.loss.diagnosisId = this.loss.diagnosis.id;
                this.controlForDiagnosis.setValue({ name: this.loss.diagnosis.name });
                this.controlForDiagnosisCode.setValue({ code: this.loss.diagnosis.code });
                this.controlForClinic.setValue({ clinicName: this.loss.clinic.clinicName });
                this.controlForInn.setValue({ inn: this.loss.clinic.inn });
                for (const value of this.loss.medicaments) {
                    value.readonly = true;
                }
                for (const value of this.loss.procedures) {
                    value.readonly = true;
                }
                this._losesservice.getAllLossesForUser(data.assured.id).subscribe(data => {
                    this.lossListForUser = data;
                }, error => { console.log(error); });
            }, error => { console.log(error); });
        }
        this._mkb10Service.getMkbList().subscribe(data => {
            for (const value of data) {
                if (value.nodeCount === 0) {
                    this.optionsForDiagnosis.push(value);
                    this.optionsForDiagnosisModal.push(value);
                    this.optionsForDiagnosisCode.push(value);
                    this.optionsForDiagnosisCodeInModal.push(value);
                }
            }
            this.filteredOptionsForDiagnosis = this._dateToSend.makeListForFilters(this.controlForDiagnosis, 'name', this.optionsForDiagnosis, 3);
            this.filteredOptionsForDiagnosisModal = this._dateToSend.makeListForFilters(this.controlForDiagnosisInModal, 'name', this.optionsForDiagnosisModal, 3);
            this.filteredOptionsForDiagnosisCode = this._dateToSend.makeListForFilters(this.controlForDiagnosisCode, 'code', this.optionsForDiagnosisCode, 2);
            this.filteredOptionsForDiagnosisCodeInModal = this._dateToSend.makeListForFilters(this.controlForDiagnosisCodeInModal, 'code', this.optionsForDiagnosisCodeInModal, 2);
            this.showSpinerDiagnosis = false;
            this.showSpinerCode = false;
        }, error => { console.log(error); });
        this._mkb10Service.getListOfClinic().subscribe(data => {
            for (const value of data) {
                this.optionsForClinic.push(value);
                this.optionsForInn.push(value);
            }
            this.filteredOptionsForClinic = this._dateToSend.makeListForFilters(this.controlForClinic, 'clinicName', this.optionsForClinic, 0);
            this.filteredOptionsForInn = this._dateToSend.makeListForFilters(this.controlForInn, 'inn', this.optionsForInn, 0);
            this.showSpinerClinic = false;
            this.showSpinerInn = false;
        }, error => { console.log(error); });
    }
    chooseDiagnosis(option) {
    ...
    changeStatus(status) {
        ...
        this._losesservice.changeStatus(this.idOfLoss, statusToSend).subscribe(error => { console.log(error); });
    }
    sendThisLoss() {
        const lossToSend = {
        ...
        this._losesservice.postDamage(lossToSend).subscribe(data => { }, error => { console.log(error); });
    }
    sendThisNewLoss() {
        const lossToSend = {
            assuredId: this.loss.assured.id,
        ...
        this._losesservice.postDamage(lossToSend).subscribe(data => {
            this._router.navigate(['../../losses', data.id], { replaceUrl: true });
        }, error => { console.log(error); });
    }
    goToLoss(damage) {
...
let LossListComponent = class LossListComponent {
    constructor(_losesservice, _dateToSend) {
        this._losesservice = _losesservice;
        this._dateToSend = _dateToSend;
    ...
    updateList() {
        this._losesservice.getLoses(this.filtersToSend).subscribe(data => {
            this.damages = data.content;
            this.amountOfDamages = this.damages.length;
            this.typeOfAmountToShow = this._dateToSend.checkAmountToShow(this.amountOfDamages);
            for (const value of this.damages) {
    ...
    changeStatus(id, stateId) {
        const statusToSend = {
            stateId: stateId
        };
        this._losesservice.changeStatus(id, statusToSend).subscribe(resp => {
            if (resp.status === 200) {
                for (const value of this.damages) {
                    if (value.id === id) {
    ...
    deleteLoss() {
        for (const value of this.damages) {
            if (value.selected) {
                this._losesservice.deleteLoss(value.id).subscribe(data => {
                    this.updateList();
                }, error => { console.log(error); });
            }
        }
...
let PageForUserComponent = class PageForUserComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let ProductIdComponent = class ProductIdComponent {
    constructor(router, _settingsService, _dateToSend) {
        this.router = router;
        this._settingsService = _settingsService;
        this._dateToSend = _dateToSend;
        this.readonly = true;
    ...
    changeProduct() {
        ...
        this._settingsService.postProductById(this.product).subscribe(data => {
            if (this.idOfProdact[0] === 'n') {
                this.readonly = true;
                this.router.navigate(['../companies/' + this.product.insuredCompanyId + '/products/' + data], { replaceUrl: true });
            }
            else {
                this.readonly = true;
            }
    ...
    ngOnInit() {
        if (this.idOfProdact[0] === 'n') {
            ...
            this._settingsService.getProductById(this.idOfProdact).subscribe(data => {
                this.product.name = data.name;
                this.product.id = data.id;
                this.product.insuredCompanyId = data.insuredCompanyId;
                this.product.dateEnd = data.dateEnd;
                this.product.dateStart = data.dateStart;
                this.product.version = data.version;
                this.product.document = data.document;
            }, error => { console.log(error); });
        }
    }
};
...
let ProductListComponent = class ProductListComponent {
    ...
    updateList() {
        this._settingsService.getProductListByCompany(this.idOfDamage).subscribe(data => {
            this.productsList = data;
            this.amountOfProducts = data.length;
            this.typeOfLength = this._dateToSendService.checkAmountToShow(this.amountOfProducts);
            for (const product of this.productsList) {
                product.selected = false;
            }
            if (this.productsList.length === 0) {
                this.showButtonForNew = true;
            }
            else {
                this.showButtonForNew = false;
            }
    ...
    deleteCompany() {
        for (const product of this.productsList) {
            if (product.selected === true) {
                this._settingsService.deleteProductById(product.id).subscribe(data => {
                    this.updateList();
                }, error => {
                    console.log(error);
                });
    ...
    ngOnInit() {
        this.updateList();
        this._settingsService.getCompanyById(this.idOfDamage).subscribe(data => {
            this.nameOfCompany = data.companyName;
        }, error => {
            console.log(error);
        });
    }
...
let ProgramIdComponent = class ProgramIdComponent {
    constructor(_settingsservice, router, _dateToSend) {
        this._settingsservice = _settingsservice;
        this.router = router;
        this._dateToSend = _dateToSend;
        this.readonly = true;
    ...
    changeProgram() {
        ...
        this._settingsservice.postProgramtById(programToSend).subscribe(data => {
            this.readonly = true;
            this.program.id = data;
            if (this.idOfProgram[0] === 'n') {
                this.idOfProgram = data;
                this.router.navigate([this.router.url.slice(0, -3), data], { replaceUrl: true });
            }
        }, error => {
            console.log(error);
        });
...
let ProgramListComponent = class ProgramListComponent {
    ...
    updateList() {
        this._settingsservice.getProgramListByProduct(this.idOfProgram).subscribe(data => {
            this.programsList = data;
            this.amountOfPrograms = data.length;
            this.typeOfLength = this._dateToSendService.checkAmountToShow(this.amountOfPrograms);
    ...
    deleteSelected() {
        for (const program of this.programsList) {
            if (program.selected === true) {
                this._settingsservice.deleteProgramById(program.id).subscribe(data => {
                    this.updateList();
                }, error => {
                    console.log(error);
                });
    ...
    ngOnInit() {
        this._settingsservice.getProductById(this.idOfProgram).subscribe(data => {
...
let DateToSendService = class DateToSendService {
    constructor() { }
    makeDateToSend(date) {
    ...
    _filterCode(name, options) {
        const filterValue = name.toLowerCase();
        return options.filter(option => option.code.toLowerCase().indexOf(filterValue) === 0);
    }
    _filterClinic(name, options) {
        const filterValue = name.toLowerCase();
        return options.filter(option => option.clinicName.toLowerCase().indexOf(filterValue) === 0);
    }
    _filterInn(name, options) {
        const filterValue = name.toLowerCase();
        return options.filter(option => option.inn.toString().toLowerCase().indexOf(filterValue) === 0);
    }
    _filterCompanyName(name, options) {
        const filterValue = name.toLowerCase();
        return options.filter(option => option.companyName.toLowerCase().indexOf(filterValue) === 0);
    }
    makeListForFilters(control, type, options, amount) {
        if (type === 'name') {
            return control.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(value => typeof value === 'string' ? value : value.name), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(name => name.length >= amount ? this._filter(name, options) : []));
        }
        else if (type === 'code') {
            return control.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(value => typeof value === 'string' ? value : value.code), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(code => code.length >= amount ? this._filterCode(code, options) : []));
        }
        else if (type === 'clinicName') {
            return control.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(value => typeof value === 'string' ? value : value.clinicName), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(clinicName => clinicName.length >= amount ? this._filterClinic(clinicName, options) : []));
        }
        else if (type === 'inn') {
            return control.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(value => typeof value === 'string' ? value : value.inn), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(inn => inn.length >= amount ? this._filterInn(inn, options) : []));
        }
        else if (type === 'companyName') {
            return control.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["startWith"])(''), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(value => typeof value === 'string' ? value : value.companyName), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(companyName => companyName.length >= amount ? this._filterCompanyName(companyName, options) : []));
        }
    }
    displayFn(user) {
...
let Mkb10Service = class Mkb10Service {
    constructor(httpClient) {
        this.httpClient = httpClient;
        this.serverUrl = src_environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].serverUrl;
        this.httpClient.get(this.serverUrl + '/classifiers/icd10').subscribe(data => {
            this.treeOfMkb = data;
        }, error => {
            console.log(error);
        });
        this.httpClient.get(this.serverUrl + '/classifiers/icd10?type=list').subscribe(data => {
            this.listOfMkb = data;
        }, error => {
            console.log(error);
        });
        this.httpClient.get(this.serverUrl + '/classifiers/medicaments?paged=false').subscribe(data => {
            this.listOfMed = data;
        }, error => {
            console.log(error);
        });
        this.httpClient.get(this.serverUrl + '/classifiers/procedures?paged=false').subscribe(data => {
            this.listOfService = data;
        }, error => {
            console.log(error);
        });
        this.httpClient.get(this.serverUrl + '/classifiers/medicaments/groups?paged=false').subscribe(data => {
            this.listOfMedGroup = data;
        }, error => {
            console.log(error);
        });
        this.httpClient.get(this.serverUrl + '/classifiers/procedures/groups?paged=false').subscribe(data => {
            this.listOfServiceGroup = data;
        }, error => {
            console.log(error);
        });
        this.httpClient.get(this.serverUrl + '/classifiers/clinics').subscribe(data => {
    ...
    justReturn(list) {
        const listForReturning = new rxjs__WEBPACK_IMPORTED_MODULE_3__["Observable"](observer => {
            setTimeout(() => {
                observer.next(list);
            }, 100);
        });
...
let SettingTreeMedAddComponent = class SettingTreeMedAddComponent {
    constructor(_mkb10Service, _dateToSend) {
        this._mkb10Service = _mkb10Service;
        this._dateToSend = _dateToSend;
...
let SettingTreeMedComponent = class SettingTreeMedComponent {
    constructor() {
        this.onChanged = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
    }
    makeDefault() {
...
let SettingTreeProcedureAddComponent = class SettingTreeProcedureAddComponent {
    constructor(_mkb10Service, _dateToSend) {
        this._mkb10Service = _mkb10Service;
        this._dateToSend = _dateToSend;
        this.onSave = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
...
let SettingTreeProcedureComponent = class SettingTreeProcedureComponent {
    constructor() {
        this.onChanged = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
    }
    makeDefault() {
...
let SettingTreeComponent = class SettingTreeComponent {
    constructor(_mkb10Service, router) {
        this._mkb10Service = _mkb10Service;
        this.router = router;
        this.settings = {
        ...
        this.treeFlattener = new _angular_material_tree__WEBPACK_IMPORTED_MODULE_4__["MatTreeFlattener"](this._transformer, node => node.level, node => node.expandable, node => node.nodes);
...
let UserExceptionMedGroupsComponent = class UserExceptionMedGroupsComponent {
    constructor(_dateToSend, _mkb10Service) {
        this._dateToSend = _dateToSend;
        this._mkb10Service = _mkb10Service;
        this.onUpdate = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
        this.medGroups = [];
...
let UserExceptionMedComponent = class UserExceptionMedComponent {
    constructor(_mkb10Service, _dateToSend) {
        this._mkb10Service = _mkb10Service;
        this._dateToSend = _dateToSend;
        this.onUpdate = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
...
let UserExceptionProcedureGroupsComponent = class UserExceptionProcedureGroupsComponent {
    constructor(_dateToSend, _mkb10Service) {
        this._dateToSend = _dateToSend;
        this._mkb10Service = _mkb10Service;
        this.onUpdate = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
    ...
    ngOnInit() {
        ...
        this._mkb10Service.getListOfServiceGroup().subscribe(data => {
            for (const value of data.content) {
                this.optionsForProcedureGroup.push(value);
            }
            this.filteredOptionsForProcedureGroup = this._dateToSend.makeListForFilters(this.controlForProcedureGroup, 'name', this.optionsForProcedureGroup, 0);
            this.showSpinerProcedureGroup = false;
        }, error => { console.log(error); });
    }
};
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
...
let UserExceptionProcedureComponent = class UserExceptionProcedureComponent {
    constructor(_mkb10Service, _dateToSend) {
        this._mkb10Service = _mkb10Service;
        this._dateToSend = _dateToSend;
        this.onUpdate = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
 ...
 */
let UserExceptionTreeMkbComponent = class UserExceptionTreeMkbComponent {
    constructor(database) {
         ...
         */
        this.transformer = (node, level) => {
            let flatNode = this.nestedNodeMap.has(node) && this.nestedNodeMap.get(node).name === node.name ? this.nestedNodeMap.get(node) : new TodoItemFlatNode();
...
let UserExceptionComponent = class UserExceptionComponent {
    constructor() {
        this.onSave = new _angular_core__WEBPACK_IMPORTED_MODULE_1__["EventEmitter"]();
...
let UserIdLossComponent = class UserIdLossComponent {
    constructor(_losesService, router) {
        this._losesService = _losesService;
        this.router = router;
        this.listOfDamages = [];
    ...
    ngOnInit() {
        if (this.idOfUser[0] !== 'n') {
            this._losesService.getAllLossesForUser(this.idOfUser).subscribe(data => {
                if (data.length !== 0) {
                    this.listOfDamages = data;
                    this.showListOfDamages = true;
                }
            }, error => {
                console.log(error);
            });
...
let UserIdComponent = class UserIdComponent {
    constructor(_usersservice, router, _snackBar, _losesService, _dateToSend, _mkb10Service, _settingsService) {
        this._usersservice = _usersservice;
        this.router = router;
        ...
        this.treeControl = new _angular_cdk_tree__WEBPACK_IMPORTED_MODULE_8__["FlatTreeControl"](node => node.level, node => node.expandable);
        this.treeFlattener = new _angular_material_tree__WEBPACK_IMPORTED_MODULE_9__["MatTreeFlattener"](this.transformer, node => node.level, node => node.expandable, node => node.nodes);
    ...
    sendUser() {
        ...
        if (this.modalUser.firstName === '' ||
            ...
            this._usersservice.sendUser(userToSend).subscribe(data => {
                this.readonly = true;
                if (this.newUser === true) {
                    this.router.navigate(['../users', data], { replaceUrl: true });
                }
            }, error => {
                console.log(error);
            });
    ...
    saveExceptions() {
        ...
        this._usersservice.postUserExceptionsById(this.assuredExceptionItem, this.modalUser.id).subscribe(data => {
            this.assuredExceptionItem = data;
        }, error => {
            console.log(error);
        });
    ...
    getExceptions() {
        this._usersservice.getUserExceptionsById(this.modalUser.id).subscribe(data => {
            this.assuredExceptionItem = data;
        }, error => {
            console.log(error);
        });
    }
    chooseCompany(option) {
        this.modalUser.insuredCompanyProductProgramId = null;
        this.showSpinerProduct = true;
        this.showSpinerProgram = true;
    ...
    chooseProgram(option) {
        this.modalUser.insuredCompanyProductProgramId = option.id;
    }
    getProductList(id) {
        this._settingsService.getProductListByCompany(id).subscribe(data => {
            for (const value of data) {
                this.optionsForProduct.push(value);
            }
            this.filteredOptionsForProduct = this._dateToSend.makeListForFilters(this.controlForProduct, 'name', this.optionsForProduct, 0);
            this.showSpinerProduct = false;
            console.log(data);
        }, error => {
            console.log(error);
        });
    ...
    getProgramList(id) {
        this._settingsService.getProgramListByProduct(id).subscribe(data => {
            for (const value of data) {
                this.optionsForProgram.push(value);
            }
            this.filteredOptionsForProgram = this._dateToSend.makeListForFilters(this.controlForProgram, 'name', this.optionsForProgram, 0);
            this.showSpinerProgram = false;
            console.log(data);
        }, error => {
            console.log(error);
        });
    ...
    ngOnInit() {
        if (this.idOfUser[0] !== 'n') {
            this.newUser = false;
            this._usersservice.getUserById(this.idOfUser).subscribe(data => {
                this.modalUser = data;
                this.controlForCompany.setValue({ companyName: this.modalUser.insuredCompanyName });
                this.controlForProduct.setValue({ name: this.modalUser.insuredCompanyProductName });
                this.controlForProgram.setValue({ name: this.modalUser.insuredCompanyProductProgramName });
                this.getExceptions();
            }, error => {
                console.log(error);
            });
        ...
        this._settingsService.getCompanyList([]).subscribe(data => {
            for (const value of data.content) {
                this.optionsForCompany.push(value);
            }
            this.filteredOptionsForCompany = this._dateToSend.makeListForFilters(this.controlForCompany, 'companyName', this.optionsForCompany, 0);
            this.showSpinerCompany = false;
        }, error => {
            console.log(error);
        });
    }
...
let UserListComponent = class UserListComponent {
    constructor(_usersservice, _dateTosend, router) {
        this._usersservice = _usersservice;
        this._dateTosend = _dateTosend;
        this.router = router;
    ...
    getUserList() {
        this._usersservice.postUsers(this.filtersToSend).subscribe(data => {
            this.users = data.content;
            this.amountOfUsers = this.users.length;
            this.typeOfAmountToShow = this._dateTosend.checkAmountToShow(this.amountOfUsers);
    ...
    getUserListByCompanyId() {
        this._usersservice.postUsersByCompanyId(this.filtersToSend, this.idForFilter).subscribe(data => {
            this.users = data.content;
            this.amountOfUsers = this.users.length;
            this.typeOfAmountToShow = this._dateTosend.checkAmountToShow(this.amountOfUsers);
            for (const value of this.users) {
    ...
    getUserListByProductId() {
        this._usersservice.postUsersByProductId(this.filtersToSend, this.idForFilter).subscribe(data => {
            this.users = data.content;
            this.amountOfUsers = this.users.length;
            this.typeOfAmountToShow = this._dateTosend.checkAmountToShow(this.amountOfUsers);
            for (const value of this.users) {
    ...
    getUserListByProgramId() {
        this._usersservice.postUsersByProgramId(this.filtersToSend, this.idForFilter).subscribe(data => {
            this.users = data.content;
            this.amountOfUsers = this.users.length;
            this.typeOfAmountToShow = this._dateTosend.checkAmountToShow(this.amountOfUsers);
            for (const value of this.users) {
    ...
    deleteSelectedUsers() {
        for (const value of this.users) {
            if (value.selected) {
                this._usersservice.deleteUserById(value.id).subscribe(data => {
                    this.getUserList();
                }, error => {
                    console.log(error);
                });
